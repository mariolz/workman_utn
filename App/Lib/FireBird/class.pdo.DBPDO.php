...
class DBPDO {
	...
	private $char_set         = 'GB2312';
	private $out_char_set     = 'UTF-8';
	private $conn_id          = null;
	static private $_mem_obj  = null;
    function __construct() {
    	if(GOLBALCACHE) {
    	    self::$_mem_obj = new Cache();
    	}
        $this->connect();
    }
    // --------------------------------------------------------------------
    ...
    function connect() {
        ...
        $this->conn_id = @ibase_connect($hoststring, $this->username, $this->password);
    }
    // --------------------------------------------------------------------
    /**
    ...
    function pconnect() {
        $hoststring    = $this->hostname . ':' . $this->database;
        $this->conn_id =  @ibase_pconnect($hoststring, $this->username, $this->password, $this->char_set, $this->dialect);
    }
    // --------------------------------------------------------------------
    /**
     * Reconnect
     ...
     * 获取字段信息
     */
    function getFieldsInfo($sql) {
    	$rs       = $this->query($sql);
    	$col_info = array();
    	$num      = ibase_num_fields($rs);
    	for($i=0; $i<$num;$i++) {
     ...
     * 获取结果集
     */
    function fetchAll($sql,$values = null,$cache=false) {
    	$dataArr = array();
    	if(GOLBALCACHE && $cache) {
    		$mem_key = $sql;
    		//var_dump(self::$_mem_obj->get($mem_key));
    		if(self::$_mem_obj->get($mem_key)) {
    			return self::$_mem_obj->get($mem_key);
    		} else {
    			$final_res = array();
    			...
    			$final_res = $this->converCharset($dataArr);
    			unset($dataArr);
    			self::$_mem_obj->set($mem_key,$final_res);
    			return $final_res;
    		}
    	} else {
    		$final_res = array();
     ...
     * 获取一条结果
     */
    function fetch($sql,$values = null,$cache=false) {
    	//$dataArr = array();
    	if(GOLBALCACHE && $cache) {
    		$mem_key = $sql;
    		//var_dump(self::$_mem_obj->get($mem_key));
    		if(self::$_mem_obj->get($mem_key)) {
    			...
    			$final_res = $this->converCharset($row,$this->char_set,"UTF-8");
    			self::$_mem_obj->set($mem_key,$final_res);
    			return $final_res;
    		}   		
    	} else {
    		...
    		$res = $this->_execute($sql);
    		$row = ibase_fetch_assoc ($res);
    		$final_res = $this->converCharset($row);
    		return $final_res;
    	}
     ...
     * @param 转换后的数据的编码 $out
     */
    function converCharset ($data) {
    	return is_array($data)?array_map(array($this,'converCharset') , $data):mb_convert_encoding($data,$this->out_char_set,$this->char_set);
    	/*
    	$res = array();
    	foreach($data as $k=>$v) {
    		foreach($v as $kk=>$vv) {
    			$res[$k][$kk] = mb_convert_encoding($vv,$out,$in);
    		}
    	}
    	unset($data);
    	return $res;*/
